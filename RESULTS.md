...
## Extension: a real, live, external site

Everything above is on an app I built myself, which means I know its structure going in — a fair objection is "of course the numbers look clean, you control both sides." To test whether the mechanism generalizes off my own turf, I ran a second, smaller (4-task, read-only) benchmark against **[automationexercise.com](https://automationexercise.com)**, a real, live, third-party e-commerce site built specifically for automation practice. It was the right pick because it's the rare live site that (a) needs no login or payment for search/browse tasks, so nothing in the safety rules around accounts or transactions applies, and (b) publishes a genuine [public API](https://automationexercise.com/api_list) alongside its UI — a true external analog to "agent-native capabilities," not something I built to make a point.

4 read-only tasks: search for "dress", browse Women→Dress, browse by brand "Polo", and look up one product's price by id.

| Metric | DOM UI | Public API |
|---|---|---|
| Task success | 4/4 | 4/4 |
| Agent actions | 32 | 2 |
| Wasted actions | 6 (18.8%) | 0 |
| HTTP requests | ~7 page loads | 2 |

Two things came out of this that the controlled demo app couldn't have shown:

**A live ad overlay silently ate two clicks.** Mid-task, a third-party ad (first a blank unit, then a Texas Instruments recruitment ad) rendered on top of the sidebar and absorbed clicks aimed at the "Polo" brand link — confirmed only by taking a screenshot, since the accessibility tree gave no indication anything was wrong. This is not a contrived failure mode; it's the single largest source of DOM-mode action cost in this run (6 of 32 actions), and it's structurally impossible to encounter on a controlled demo app with no ad inventory. **This is arguably the single most externally-valid finding in the whole project** — render-timing races (from the first benchmark) are a plausible generalization from a clean local server; getting your click stolen by a recruitment ad is a failure mode you can only find by actually leaving your own sandbox.

**One API response answered three tasks.** `GET /api/productsList` returns brand and category metadata inline for every product. That one cached response, fetched once for the "browse by category" task, also answered the "browse by brand" and "product lookup" tasks with zero additional HTTP calls — the DOM equivalent needed three separate page navigations, because the rendered product-listing page doesn't show brand/category as visible text (you only see it once you're on a category page, brand page, or the product's own detail page). This is a distinct mechanism from anything in the first benchmark: **a sufficiently rich structured response can serve multiple logically-separate DOM navigations from a single fetch**, which is a capability a browsing agent structurally cannot replicate no matter how efficient its clicking is.

Action-count ratio here is 16x (32 vs. 2) — notably higher than the demo app's 5.6x. I'm not claiming 16x as a more accurate number; if anything it's noisier (N=4, one run, one outlier task dominated by an unlucky ad). What I'd defend is the direction and the mechanism: a real site added a real-world failure mode (ad interception) that only shows up outside a sandbox, and a real published API showed the same "structured response substitutes for multiple navigations" pattern the demo app's T5/T7 comparison predicted, just via a different concrete mechanism (rich per-record metadata rather than a returned id). Raw data: [results/real-site/dom-run.json](results/real-site/dom-run.json), [results/real-site/api-run.json](results/real-site/api-run.json).
...
- **The real-site extension is N=4, one run, read-only only.** I deliberately excluded any task requiring login, cart, or checkout (account creation and payment are both outside what I'll do without explicit sign-off, and neither was needed to test the core question). One DOM task (RT4) navigated directly to a URL pattern I'd already observed from an earlier page read rather than clicking through — a minor, disclosed deviation from "pure clicking," consistent with how a DOM agent would behave after having already explored the site once. The 16x ratio from that run is a single noisy data point, not a number I'd defend as precisely as the demo app's 5.6x.
